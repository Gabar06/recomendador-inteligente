...
// Íconos SVG
const DownloadIcon = () => (<span role="img" aria-label="descargar">📥</span>);
const EditIcon = () => (<span role="img" aria-label="editar">✏️</span>);
const DeleteIcon = () => (<span role="img" aria-label="eliminar">🗑️</span>);
const SearchIcon = () => (
  <svg height="25" width="25" viewBox="0 0 24 24" style={{verticalAlign: 'middle'}}>
    <circle cx="11" cy="11" r="8" stroke="green" strokeWidth="2" fill="none"/>
    <line x1="16" y1="16" x2="22" y2="22" stroke="purple" strokeWidth="2"/>
  </svg>
);
...
function BibliotecaVirtual() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/libros/')
      ...
      .then(data => {
        // Mapea 'año' a 'anio'
        const librosMapeados = data.map(libro => ({
          ...libro,
          anio: libro['año']
        }));
        setLibros(librosMapeados);
      })
      .catch(e => console.error("Error trayendo libros:", e));
  }, []);
