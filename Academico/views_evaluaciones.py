...
def evaluaciones_view(request):
    ...
    acent_values = []
    acent_slugs = ["acentuacion1","acentuacion2","acentuacion3"]
...
def evaluaciones_docente_view(request):
    ...
    for est in page_obj.object_list:
        # ACENTUACIÓN
        acent_values = []
        for res in MultipleChoiceResult.objects.filter(user=est, exercise_slug__in=["acentuacion1","acentuacion2","acentuacion3"]):
...
###########
# Vista individual de educando

@login_required
@role_login_required(Usuario.DOCENTE, login_url_name="login_docente")
def evaluaciones_docente_individual_report(request, user_id: int) -> HttpResponse:
    """
    Genera un PDF con el reporte de evaluaciones para el educando indicado
    (misma lógica que evaluaciones_report pero para cualquier estudiante).
    """
    User = get_user_model()
    selected = get_object_or_404(User, pk=user_id, role=Usuario.ESTUDIANTE)

    # --- Reutilizamos la misma lógica que en evaluaciones_report ---
    # Acentuación
    acent_values = []
    acent_slugs = ["acentuacion1", "acentuacion2", "acentuacion3"]
    acent_mc = MultipleChoiceResult.objects.filter(user=selected, exercise_slug__in=acent_slugs)
    acent_values += [res.percentage for res in acent_mc]
    acento_res = ResultSummary.objects.filter(user=selected).order_by("-created_at").first()
    if acento_res:
        acent_values.append(acento_res.percentage)
    acento_pct = _average_percentage(acent_values)
    ...
    # Puntuación
    punct_slugs = ["puntuacion1", "puntuacion2", "puntuacion3"]
    punct_mc_results = MultipleChoiceResult.objects.filter(user=selected, exercise_slug__in=punct_slugs)
    punct_values = [res.percentage for res in punct_mc_results]
    punct_final = PunctuationResult.objects.filter(user=selected).order_by("-created_at").first()
    if punct_final:
        punct_values.append(punct_final.percentage)
    puntuacion_pct = _average_percentage(punct_values)
    ...
    # Mayúsculas
    mayus_slugs = ["mayus1", "mayus2", "mayus3"]
    mayus_results = MultipleChoiceResult.objects.filter(user=selected, exercise_slug__in=mayus_slugs)
    mayus_values = [res.percentage for res in mayus_results]
    mayus_pct = _average_percentage(mayus_values)

    # Reglas de las letras
    letras_slugs = ["letras1", "letras2", "letras3"]
    letras_results = MultipleChoiceResult.objects.filter(user=selected, exercise_slug__in=letras_slugs)
    letras_values = [res.percentage for res in letras_results]
    letras_pct = _average_percentage(letras_values)

    # Evaluación final
    final_res = MultipleChoiceResult.objects.filter(user=selected, exercise_slug="evaluacionfinal").order_by("-created_at").first()
    final_pct = final_res.percentage if final_res else None

    # Construcción de tabla de resultados (igual que en evaluaciones_report)
    results = {
        "Acentuación": (f"{acento_pct:.0f}%" if acento_pct is not None else "N/A", _classify_domain(acento_pct)),
        "Puntuación": (f"{puntuacion_pct:.0f}%" if puntuacion_pct is not None else "N/A", _classify_domain(puntuacion_pct)),
        "Mayúscula y Minúscula": (f"{mayus_pct:.0f}%" if mayus_pct is not None else "N/A", _classify_domain(mayus_pct)),
        "Reglas de las Letras": (f"{letras_pct:.0f}%" if letras_pct is not None else "N/A", _classify_domain(letras_pct)),
        "Evaluación final": (f"{final_pct:.0f}%" if final_pct is not None else "N/A", _classify_domain(final_pct)),
    }
    ...
    if _HAS_REPORTLAB:
        ...
        c.setFont("Helvetica", 12)
        c.setFillColor(colors.black)
        c.drawString(margin_x, info_y, f"Nombre: {selected.nombre + ' ' + (selected.apellido if selected.apellido else '')}")
        info_y -= 18
        user_id_val = getattr(selected, 'cedula', '')
        c.drawString(margin_x, info_y, f"Cédula: {user_id_val}")
        ...
        with PdfPages(buffer) as pdf:
            ...
            ax.text(0.1, info_y, 'Información del Educando', fontsize=13, weight='bold', color=green_dark,
                    transform=ax.transAxes)
            info_y -= 0.04
            ax.text(0.1, info_y, f'Nombre: {selected.nombre + " " + selected.apellido or ""}', fontsize=11, color='black',
                    transform=ax.transAxes)
            info_y -= 0.03
            user_id_val = getattr(selected, 'cedula', '')
            ax.text(0.1, info_y, f'Cédula: {user_id_val}', fontsize=11, color='black', transform=ax.transAxes)
    ...
    response["Content-Disposition"] = 'attachment; filename="Reporte_de_Evaluaciones.pdf"'
    return response
    # --- Generación PDF: copiar exactamente la sección que usás en evaluaciones_report ---
    # (puedes copiar la implementación con ReportLab o Matplotlib que ya tengas
    # en el mismo archivo; aquí se asume que ya existe lógica para crear pdf_data)
    # Ejemplo: reutilizar el mismo bloque que crea `pdf_data` y devuelve HttpResponse.

    # Para no duplicar demasiado el código en este ejemplo, llamo a la
    # función interna que ya generaste; si no tenés función reutilizable,
    # pega aquí el bloque de ReportLab que usás en evaluaciones_report.

    # --- Ejemplo simplificado: llamar a la función existente si la convertiste en helper
    pdf_response = _build_evaluaciones_pdf_response_for_user(selected, results)  # ver nota abajo
    return pdf_response
...
@login_required
@role_login_required(Usuario.ADMINISTRADOR, login_url_name="login_administrador")
def evaluaciones_administrador_individual_report(request, user_id: int) -> HttpResponse:
    """
    Genera un PDF con el reporte de evaluaciones para el educando indicado
    (misma lógica que evaluaciones_report pero para cualquier estudiante).
    """
    User = get_user_model()
    selected = get_object_or_404(User, pk=user_id)
    ...
    if _HAS_REPORTLAB:
        ...
        c.setFont("Helvetica-Bold", 14)
        c.setFillColor(green_dark)
        c.drawString(margin_x, info_y, "Información del Usuario")
        info_y -= 20
        c.setFont("Helvetica", 12)
        c.setFillColor(colors.black)
        c.drawString(margin_x, info_y, f"Nombre: {selected.nombre + ' ' + (selected.apellido if selected.apellido else '')}")
